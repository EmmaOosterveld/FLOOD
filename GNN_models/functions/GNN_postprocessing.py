...
def output_animation(d_dem, d_output, d_data, id, dataset, size, pos):
  ...
  def update(timestep):
      ...
      fig.suptitle(f'Predictions for dataset {dataset}[{id}] at Timestep - {timestep / 2} hours')
...
def test_loss(test_dataset, model, WD_scaler, loss_function):
  """
  ...
  for i in test_dataset:
    ...
    test_loss += loss_function(torch.Tensor(denormalized_test_output), torch.Tensor(denormalized_data[:, :output_size]))
